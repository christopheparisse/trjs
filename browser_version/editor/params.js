...
trjs.param = {
	...
	/* TODO map of the keyboard commands
	this.map_playpause = 'F1';
	this.map_backward = 'F2';
	this.map_forward = 'F3';
	F1 = keyCode + (0:nothing + 1000:shift + 2000:alt + 4000:ctrl + 8000:cmd)
	*/
	...
	features: {
		...
		settingsTPW: function() { this.settings = '-textual-partition-wave-'; },
		settingsTP: function() { 
		...
		settingsT: function() { this.settings = '-textual-'; },
		value: function(val) {
		...
		wave: function() { return this.settings.indexOf('-wave-') < 0 ? false : true; },
	},
	...
	synchro: {
		...
		block: function(p) { return this.check(p, 'block'); },
		control: function(p) { return this.check(p, 'control'); },
		free: function(p) { return this.check(p, 'free'); },
		init: function(p) {
