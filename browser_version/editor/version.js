 ...
 * lists all versionning and system configuration variables
 */
...
version.location = 'local';
version.server = 'nodejs';
...
version.setPhp = function(port, location, home) {
	...
	version.location = (location) ? location : 'remote';
	version.server = 'php';
...
version.setNodeJs = function(port, location, home) {
	...
	if (port === undefined)
		...
		version.HTTP_PORT = port;
	version.location = (location) ? location : 'local';
	version.server = 'nodejs';
	if (home !== undefined) version.home = home;
};
...
version.WEB_ADDRESS = function() { return 'http://localhost:' + version.HTTP_PORT; };
...
version.trjsLoc = function () {
	if (__dirname !== undefined) {
		var htmlTrjsPath = __dirname.replace(/\\/,'/','g') + "/..";
	} else
...
version.ffmpegdirLoc = function () {
	if (__dirname !== undefined) {
		var htmlTrjsPath = __dirname.replace(/\\/,'/','g') + "/../tools";
	} else
		var htmlTrjsPath = process.cwd().replace(/\\/,'/','g') + "/tools";
	return htmlTrjsPath;
};
