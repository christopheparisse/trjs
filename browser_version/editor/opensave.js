...
trjs.io = (function() {
	...
	function doSaveAs() {
		innerSave();
		trjs.filetree.initChooseFile("transcriptsaveas", "transcript");
		/*	bootbox.prompt("What is the name of the new file?", function(result) {
		 if (result === null) {
			 return;
		 } else {
			...
			var withExt = false;
			for (var i in exts) {
				if (exts[i] && result.toLowerCase().endsWith(exts[i])) {
					withExt = true;
					break;
				};
			}
	...
	function exportCsv() {
        ...
        trjs.data.transcriptInner = null;
		trjs.system.saveFile({data: s, name: codefn.encodeFilename(fn)},
		function (params) {
...
/*
		...
		// trjs.system.saveBlob({data: blob, name: (trjs.data.recordingRealFile() ? trjs.data.recordingRealFile() : "export") + '.csv'});
*/
	}
		...
		trjs.editor.testNotSave( function(yesno) {
			if (yesno === true ) {  // the user does not want to save the modified file or the file is not modified since last save
				...
				if (ext === '.cha') {
					...
					// console.log(data);
					trjs.system.formatToTei(data, function(r) {
							// console.log(r);
	...
	function serverLoadTeiml(name, loadMediaAfter, callback) {
		...
		return serverLoadFile(name, function(err, data) {
			if (err === 1) {
				...
				if (name.toLowerCase().lastIndexOf('.xml') === name.length - 4) {
					trjs.data.doc = data;
				} else {
					// get XML ready
					var parser = new DOMParser();
	...
	function serverLoadFile(name, callback) {
		...
		var nn = codefn.encodeFilename(name);
		$.get(nn).done(function(data) {
			callback(0, data);
		}).fail(function(data) {
			callback(2, data.responseText);
		});
	}
	...
	function setAudio() {
		...
		$('#media-display').html('<audio id="audio-display" controls preload="auto" controls="controls"></source><source id="mp3-src" src="" type="audio/mp3"><source id="wav-src" src="" type="audio/wav"></source><source id="oga-src" src="" type="audio/ogg"></source></audio>');

        window.addEventListener('blur', function() {
                var mediaPlayer = document.getElementById("audio-display");
	...
	function setAudioFileUniq(name) {
		...
		$('#media-display').on('error', function failed(e) {
			// audio playback failed - show a message saying why
			// to get the source of the audio element use $(this).src
			switch (e.target.error.code) {
			case e.target.error.MEDIA_ERR_ABORTED:
	...
	function setVideo() {
		...
		$('#media-display').html('<video id="video-display" preload="auto" controls="controls"><source id="webm-src" src="" type="video/webm"></source><source id="mp4-src" src="" type="video/mp4"></source><source id="ogv-src" src="" type="video/ogg"></source></video>');

        window.addEventListener('blur', function() {
                var mediaPlayer = document.getElementById("video-display");
                if (mediaPlayer) mediaPlayer.pause();
        });
	}
