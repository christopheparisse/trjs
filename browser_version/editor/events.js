...
function testMark(s) {
	return (s.indexOf(trjs.data.MARK) === 0) ? true : false;
}
...
function checkOverlap(sel) {
	...
	while (p && p.length>0) {
		...
		var pEnd = lineGetCell(p, trjs.data.TECOL);
	...
	while (p && p.length>0) {
		var pLoc = trimMark(trjs.transcription.getCode(p));
		var pStart = lineGetCell(p, trjs.data.TSCOL);
...
function goCheck() {
	...
	for (var i = 0; i < tablelines.length-1; i++) {
		...
		var loc = trjs.transcription.getCode(sel);
		// var type = trjs.transcription.getType(sel);
		if (loc === '---' || testMark(loc)) {
			trjs.data.check.push(i);
			trjs.data.checkCount++;
		} else if (checkOverlapNext(sel)) {
			trjs.data.check.push(i);
			trjs.data.checkCount++;
		} else if (loc === '-div-' && trjs.dataload.checkstring(lineGetCell(sel, trjs.data.TRCOL)) !== '') {
			trjs.data.check.push(i);
			trjs.data.checkCount++;
		}
	}
...
function checkFinal() {
	...
	if (trjs.data.checkCount>0) {
		...
		trjs.events.goToLine(trjs.data.check[0]+1);
		trjs.editor.showCheck(true);
	} else
		trjs.log.alert(trjs.messgs.checknoerror, 'normal');
}
...
function prevCheck() {
	if (trjs.data.check.length > 0 && trjs.data.checkCount > 0) {
		if ( trjs.data.checkPos > 0 ) {
			...
			trjs.events.goToLine(trjs.data.check[trjs.data.checkPos]+1);
		} else {
			$('#check-posx').text(1);
			trjs.events.goToLine(trjs.data.check[0]+1);
		}
...
function nextCheck() {
	if (trjs.data.check.length > 0 && trjs.data.checkCount > 0) {
		if ( trjs.data.checkPos < trjs.data.checkCount-1) {
			...
			trjs.events.goToLine(trjs.data.check[trjs.data.checkCount-1]+1);
		}
	}
}
...
function checkLoc(event) {
	...
	if ( type === '---' ) {
		...
		if (!testMark(event.target.textContent))
			trjs.transcription.setCode(p, setMark(event.target.textContent));
...
return {
	checkFinal: function() { return checkFinal(); },
	checkLine: function(e) { return checkLine(e); },
	checkLoc: function(p) { return checkLoc(p); },
	checkTime: function(p) { return checkTime(p); },
	clickLoc: function(p) { return clickLoc(p); },
	copyLine: function(e) { return copyLine(e); },
	createRowAfterWith: function(s, t, l, ts, te, tr) { return createRowAfterWith(s, t, l, ts, te, tr); },
	ctrlEnd: function(e,p) { return ctrlEnd(e, p); },
	ctrlHome: function(e,p) { return ctrlHome(e, p); },
	deleteLine: function(e, p) { return deleteLine(e, p); },
	...
	deleteSelectedLine: function(p) { deleteSelectedLine(p); },
	enter: function(e, p) { return enter(e, p); },
	eventKeydown: function(p) { return eventKeydown(p); },
	eventKeypress: function(p) { return eventKeypress(p); },
	eventKeydownTranscript: function(p) { return eventKeydownTranscript(p); },
	escape: function(e) { return escape(e); },
	findLineToFollow: function(p) { return findLineToFollow(p); },
	findLineToStart: function(p) { return findLineToStart(p); },
	firstLoc: function() { return firstLoc(); },
	getSelectedLine: function() { return getSelectedLine(); },
	goCheck: function() { return goCheck(); },
	goContinuous: function(e, p) { return goContinuous(e, p); },
	goToLine: function(p) { return goToLine(p); },
	goToTime: function(o, t, p1, p2) { return goToTime(o, t, p1, p2); },
	...
	insertLineLoc: function(p, l) { insertLineLoc(p, l); },
	...
	keyUp: function(e,p) { return keyUp(e, p); },
	keyDown: function(e,p) { return keyDown(e, p); },
	keyLocUp: function(e,p) { return keyLocUp(e, p); },
	keyLocDown: function(e,p) { return keyLocDown(e, p); },
	lineGetCell: function(p1, p2) { return lineGetCell(p1, p2); },
	lineGetCellHtml: function(p1, p2) { return lineGetCellHtml(p1, p2); },
	lineSetCell: function(p1,  p2, p3) { lineSetCell(p1, p2, p3); },
	lineSetCellHtml: function(p1,  p2, p3) { lineSetCellHtml(p1, p2, p3); },
	nextCheck: function(e) { return nextCheck(e); },
	pageUp: function(e,p) { return pageUp(e, p); },
	pageDown: function(e,p) { return pageDown(e, p); },
	prevCheck: function(e) { return prevCheck(e); },
	protectLine: function(e) { protectLine(e); },
	putTime: function(p) { return putTime(p); },
	...
	runCurrentLine: function(e, p) { return runCurrentLine(e, p); },
	runThreeLines: function(e, p) { return runThreeLines(e, p); },
	...
	setEnd: function(e) { return setEnd(e); },
	...
	setMark: function(p) { return setMark(p); },
	...
	setSelectedLine: function(p, origin) { return setSelectedLine(p, origin); },
	setStart: function(e) { return setStart(e); },
	...
	setTimeSelectedLine: function(p1, p2) { return setTimeSelectedLine(p1, p2); },
	shiftTab: function(e) { return shiftTab(e); },
	...
	tab: function(e) { return tab(e); },
	testMark: function(p) { return testMark(p); },
	transcriptGotFocus: function(p) { return transcriptGotFocus(p); },
	trimMark: function(p) { return trimMark(p); },
};
})();
 ...
 */
trjs.events.setNthLoc1 = function(e) { trjs.events.setNthLoc(undefined, 1); trjs.dmz.redraw('partition'); };	// Ctrl 1
trjs.events.setNthLoc2 = function(e) { trjs.events.setNthLoc(undefined, 2); trjs.dmz.redraw('partition'); };	// Ctrl 2
trjs.events.setNthLoc3 = function(e) { trjs.events.setNthLoc(undefined, 3); trjs.dmz.redraw('partition'); };	// Ctrl 3
trjs.events.setNthLoc4 = function(e) { trjs.events.setNthLoc(undefined, 4); trjs.dmz.redraw('partition'); };	// Ctrl 4
trjs.events.setNthLoc5 = function(e) { trjs.events.setNthLoc(undefined, 5); trjs.dmz.redraw('partition'); };	// Ctrl 5
trjs.events.setNthLoc6 = function(e) { trjs.events.setNthLoc(undefined, 6); trjs.dmz.redraw('partition'); };	// Ctrl 6
trjs.events.setNthLoc7 = function(e) { trjs.events.setNthLoc(undefined, 7); trjs.dmz.redraw('partition'); };	// Ctrl 7
trjs.events.setNthLoc8 = function(e) { trjs.events.setNthLoc(undefined, 8); trjs.dmz.redraw('partition'); };	// Ctrl 8
trjs.events.setNthLoc9 = function(e) { trjs.events.setNthLoc(undefined, 9); trjs.dmz.redraw('partition'); };	// Ctrl 9
trjs.events.setNthTier1 = function(e) { trjs.events.setNthTier(undefined, 1); trjs.dmz.redraw('partition'); };  // Ctrl Alt 1
trjs.events.setNthTier2 = function(e) { trjs.events.setNthTier(undefined, 2); trjs.dmz.redraw('partition'); };  // Ctrl Alt 2
trjs.events.setNthTier3 = function(e) { trjs.events.setNthTier(undefined, 3); trjs.dmz.redraw('partition'); };  // Ctrl Alt 3
trjs.events.setNthTier4 = function(e) { trjs.events.setNthTier(undefined, 4); trjs.dmz.redraw('partition'); };  // Ctrl Alt 4
trjs.events.setNthTier5 = function(e) { trjs.events.setNthTier(undefined, 5); trjs.dmz.redraw('partition'); };  // Ctrl Alt 5
trjs.events.setNthTier6 = function(e) { trjs.events.setNthTier(undefined, 6); trjs.dmz.redraw('partition'); };  // Ctrl Alt 6
trjs.events.setNthTier7 = function(e) { trjs.events.setNthTier(undefined, 7); trjs.dmz.redraw('partition'); };  // Ctrl Alt 7
trjs.events.setNthTier8 = function(e) { trjs.events.setNthTier(undefined, 8); trjs.dmz.redraw('partition'); };  // Ctrl Alt 8
trjs.events.setNthTier9 = function(e) { trjs.events.setNthTier(undefined, 9); trjs.dmz.redraw('partition'); };  // Ctrl Alt 9
