...
function chattotei() { return version.javaLoc() + ' -cp "' + version.ffmpegdirLoc() + '/conversions.jar" fr.ortolang.teicorpo.ClanToTei'; };
function elantotei() { return version.javaLoc() + ' -cp "' + version.ffmpegdirLoc() + '/conversions.jar" fr.ortolang.teicorpo.ElanToTei'; };
function trstotei() { return version.javaLoc() + ' -cp "' + version.ffmpegdirLoc() + '/conversions.jar" fr.ortolang.teicorpo.TranscriberToTei'; };
function praattotei() { return version.javaLoc() + ' -cp "' + version.ffmpegdirLoc() + '/conversions.jar" fr.ortolang.teicorpo.PraatToTei'; };
...
function teitotrs() { return 'java -cp "' + process.cwd() + '/tools/conversions.jar" fr.ortolang.teicorpo.TeiToTranscriber'; };
function teitochat() { return 'java -cp "' + process.cwd() + '/tools/conversions.jar" fr.ortolang.teicorpo.TeiToClan'; };
function teitoelan() { return 'java -cp "' + process.cwd() + '/tools/conversions.jar" fr.ortolang.teicorpo.TeiToElan'; };
function teitopraat() { return 'java -cp "' + process.cwd() + '/tools/conversions.jar" fr.ortolang.teicorpo.TeiToPraat'; };
...
exports.save_transcript = function(file, nbs, transcript, callback) {
	...
	if (nbs<1) {
		...
		if (nbs == 1) {
			...
			try {
				fs.renameSync(filename, tf); /* rename the only file */
				fs.writeFileSync( filename, transcript );
			} catch (e) {
				callback(1, "error saving to " + filename);
			}
			...
			try {
				...
				if (fs.existsSync( filename )) {
					fs.renameSync(filename, tf1); /* rename the only file */
					etat++;
				}
				fs.writeFileSync( filename, transcript );
			} catch (e) {
