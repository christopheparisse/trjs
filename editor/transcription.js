...
//		line.find('.prop').css('padding-left', '20px');
    ...
    function updateLocNames() {
        ...
        for (var i = 0; i < tablelines.length; i++) {
            ...
            if (trjs.check.testMark(locval)) {
                marked = true;
                locval = trjs.check.trimMark(locval);
            }
            if (trjs.param.locnames === true)
                locval = trjs.template.codeToName(locval);
            else
                locval = trjs.template.nameToCode(locval);
            trjs.events.lineSetCell($(tablelines[i]), trjs.data.CODECOL, (marked === true) ? trjs.check.setMark(locval) : locval);
        }
    }
...
//	trjs.log.resetLog();
    ...
    function loadNewGrid(askformedia) {
        trjs.init.testNotSave(function (yesno) {
            if (yesno === true) {  // the user does not want to save the modified file or the file is not modified since last save
                 ...
                 * Initialisation des éléments du template au format de stockage définitif.
                 */
                trjs.data.codesxml = new Array(3);
                trjs.data.codesxml[0] = new trjs.template.TemplateInfo();
                trjs.data.codesxml[0]['code'] = 'SPK1';
                trjs.data.codesxml[0]['type'] = 'ortho';
                trjs.data.codesxml[0]['name'] = 'locutor';
                trjs.data.codesxml[0]['description'] = '';
                trjs.data.codesxml[1] = new trjs.template.TemplateInfo();
                trjs.data.codesxml[1]['code'] = 'SPK2';
                trjs.data.codesxml[1]['type'] = 'ortho';
                trjs.data.codesxml[1]['name'] = 'locutor';
                trjs.data.codesxml[1]['description'] = '';
                trjs.data.codesxml[2] = new trjs.template.TemplateInfo();
                trjs.data.codesxml[2]['code'] = 'SPK3';
                trjs.data.codesxml[2]['type'] = 'ortho';
                trjs.data.codesxml[2]['name'] = 'locutor';
                trjs.data.codesxml[2]['description'] = '';
                trjs.data.tiersxml = new Array(2);
                trjs.data.tiersxml[0] = new trjs.template.TemplateInfo();
                trjs.data.tiersxml[0]['code'] = 'ortho';
                trjs.data.tiersxml[0]['type'] = 'Symbolic Association';
    ...
    function findCode(loc) {
        if (trjs.check.testMark(loc))
            loc = trjs.check.trimMark(loc);
        if (loc === '---') {
                ...
                if (type === 'loc') {
                    ...
                    for (var t in tiers) {
                        ...
                        while (k < tablelines.length) {
                            ...
                            if (trjs.check.testMark(itier))
                                itier = trjs.check.trimMark(itier);
