...
class ContextMenuBuilder {
  ...
  constructor(spellCheckHandler) {
    ...
    let debugMode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    let processMenu = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : m => m;

    this.spellCheckHandler = spellCheckHandler;
    this.windowOrWebView = windowOrWebView || _electron.remote.getCurrentWindow();
  ...
  showPopupMenu(contextInfo) {
    ...
    return _asyncToGenerator(function* () {
      let menu = yield _this.buildMenuForElement(contextInfo);
  ...
  buildMenuForElement(info) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (info.linkURL && info.linkURL.length > 0) {
        return _this2.buildMenuForLink(info);
      }

      if (info.hasImageContents && info.srcURL && info.srcURL.length > 1) {
        return _this2.buildMenuForImage(info);
      }

      if (info.isEditable || info.inputFieldType && info.inputFieldType !== 'none') {
        return yield _this2.buildMenuForTextInput(info);
      }

      return _this2.buildMenuForText(info);
    })();
  }
  ...
  buildMenuForTextInput(menuInfo) {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this3.addSpellingItems(menu, menuInfo);
      _this3.addSearchItems(menu, menuInfo);

      _this3.addCut(menu, menuInfo);
      _this3.addCopy(menu, menuInfo);
      _this3.addPaste(menu, menuInfo);
      _this3.addInspectElement(menu, menuInfo);
      _this3.processMenu(menu);
  ...
  buildMenuForLink(menuInfo) {
    ...
    let copyLink = new MenuItem({
      ...
      click: () => {
        ...
        _electron.clipboard.writeText(isEmailAddress ? menuInfo.linkText : menuInfo.linkURL);
      }
    });
    ...
    let openLink = new MenuItem({
      ...
      click: () => {
        ...
        _electron.shell.openExternal(menuInfo.linkURL);
      }
    });
  ...
  addSpellingItems(menu, menuInfo) {
    ...
    return _asyncToGenerator(function* () {
      let target = 'webContents' in _this4.windowOrWebView ? _this4.windowOrWebView.webContents : _this4.windowOrWebView;
      ...
      if (!_this4.spellCheckHandler.currentSpellchecker) {
        return menu;
      }

      // Ensure that we have valid corrections for that word
      let corrections = yield _this4.spellCheckHandler.getCorrectionsForMisspelling(menuInfo.misspelledWord);
      ...
      corrections.forEach(function (correction) {
        let item = new MenuItem({
          label: correction,
          click: function () {
            return target.replaceMisspelling(correction);
          }
        });
        ...
        let learnWord = new MenuItem({
          ...
          click: (() => {
            var _ref = _asyncToGenerator(function* () {
              ...
              try {
                yield _this4.spellChecker.add(menuInfo.misspelledWord);
              } catch (e) {
  ...
  addSearchItems(menu, menuInfo) {
    ...
    let match = (0, _utility.matchesWord)(menuInfo.selectionText);
    if (!match || match.length === 0) {
      return menu;
    }

    if (process.platform === 'darwin') {
      let target = 'webContents' in this.windowOrWebView ? this.windowOrWebView.webContents : this.windowOrWebView;

      let lookUpDefinition = new MenuItem({
        label: `Look Up “${ (0, _utility.truncateString)(menuInfo.selectionText) }”`,
  ...
  addImageItems(menu, menuInfo) {
    let copyImage = new MenuItem({
      label: 'Copy Image',
      click: () => this.convertImageToBase64(menuInfo.srcURL, dataURL => _electron.clipboard.writeImage(_electron.nativeImage.createFromDataURL(dataURL)))
    });
    ...
    let copyImageUrl = new MenuItem({
      ...
      click: () => _electron.clipboard.writeText(menuInfo.srcURL)
    });
   ...
   */
  addInspectElement(menu, menuInfo) {
    let needsSeparator = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  ...
  convertImageToBase64(url, callback) {
    let outputFormat = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'image/png';
