...
function _load_httpExecutor() {
    return _httpExecutor = require("../util/httpExecutor");
}
...
class GitHubPublisher extends (_publisher || _load_publisher()).Publisher {
    ...
    init() {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            const releases = yield (0, (_httpExecutor || _load_httpExecutor()).githubRequest)(`/repos/${ _this.info.owner }/${ _this.info.repo }/releases`, _this.token);
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            uploadAttempt: for (let i = 0; i < 3; i++) {
                try {
                    ...
                    if (e instanceof (_httpExecutor || _load_httpExecutor()).HttpError) {
                        if (e.response.statusCode === 422 && e.description != null && e.description.errors != null && e.description.errors[0].code === "already_exists") {
                            ...
                            const assets = yield (0, (_httpExecutor || _load_httpExecutor()).githubRequest)(`/repos/${ _this2.info.owner }/${ _this2.info.repo }/releases/${ release.id }/assets`, _this2.token, null);
                            for (const asset of assets) {
                                if (asset.name === fileName) {
                                    yield (0, (_httpExecutor || _load_httpExecutor()).githubRequest)(`/repos/${ _this2.info.owner }/${ _this2.info.repo }/releases/assets/${ asset.id }`, _this2.token, null, "DELETE");
    ...
    createRelease() {
        return (0, (_httpExecutor || _load_httpExecutor()).githubRequest)(`/repos/${ this.info.owner }/${ this.info.repo }/releases`, this.token, {
            tag_name: this.tag,
            name: this.version,
    ...
    getRelease() {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            return (0, (_httpExecutor || _load_httpExecutor()).githubRequest)(`/repos/${ _this3.info.owner }/${ _this3.info.repo }/releases/${ (yield _this3._releasePromise).id }`, _this3.token);
        })();
    }
    //noinspection JSUnusedGlobalSymbols
    deleteRelease() {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            for (let i = 0; i < 3; i++) {
                try {
                    return yield (0, (_httpExecutor || _load_httpExecutor()).githubRequest)(`/repos/${ _this4.info.owner }/${ _this4.info.repo }/releases/${ release.id }`, _this4.token, null, "DELETE");
                } catch (e) {
                    if (e instanceof (_httpExecutor || _load_httpExecutor()).HttpError) {
