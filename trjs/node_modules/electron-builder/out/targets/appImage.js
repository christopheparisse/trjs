...
class AppImageTarget extends (_targetFactory || _load_targetFactory()).Target {
    constructor(ignored, packager, helper, outDir) {
        ...
        this.desktopEntry = (_bluebirdLstC2 || _load_bluebirdLstC2()).default.promisify((_uuid || _load_uuid()).v1)({ mac: false }).then(uuid => helper.computeDesktopEntry(this.options, "AppRun", null, {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            (0, (_log || _load_log()).log)(`Building AppImage for arch ${ (_metadata || _load_metadata()).Arch[arch] }`);
            ...
            if (_this.packager.options.effectiveOptionComputed != null && (yield _this.packager.options.effectiveOptionComputed([args, desktopFile]))) {
                return;
            }
            yield (0, (_util || _load_util()).exec)(process.arch === "x64" ? _path.join(appImagePath, "xorriso") : "xorriso", args);
            yield new (_bluebirdLstC2 || _load_bluebirdLstC2()).default(function (resolve, reject) {
                const rd = (0, (_fsExtraP || _load_fsExtraP()).createReadStream)(_path.join(appImagePath, arch === (_metadata || _load_metadata()).Arch.ia32 ? "32" : "64", "runtime"));
                rd.on("error", reject);
